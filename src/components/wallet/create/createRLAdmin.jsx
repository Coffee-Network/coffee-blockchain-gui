...
import { AlertDialog } from '@coffee/core';
import {
  Typography,
  Button,
  Box,
...
import { coffee_to_mojo } from '../../../util/coffee';
...
export const CreateRLAdminWallet = () => {
  ...
  function create() {
    ...
    if (
      coffeeper_input.value === '' ||
      Number(coffeeper_input.value) === 0 ||
      !Number(coffeeper_input.value) ||
      isNaN(Number(coffeeper_input.value))
    ) {
      dispatch(
        openDialog(
          <AlertDialog>
    ...
    const coffeeper = coffee_to_mojo(coffeeper_input.value);
    const coffeeper_value = Number.parseInt(Number(coffeeper));
    const userpubkey = userpubkey_input.value;
    const amount = coffee_to_mojo(amount_input.value);
    const amount_value = Number.parseInt(Number(amount));
    // var fee = coffee_to_mojo(fee_input.value);
