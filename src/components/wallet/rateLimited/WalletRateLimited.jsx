...
import { AlertDialog, Card, Flex } from '@coffee/core';
import {
  send_transaction,
...
const IncompleteCard = (props) => {
  ...
  function submit() {
    ...
    const coffeeper_input = ip_parsed.limit;
    ...
    const coffeeper_value = Number.parseInt(Number(coffeeper_input));
        ...
        <Grid item xs={12}>
          <Box display="flex">
            ...
            <Box flexGrow={1}>
              <Typography variant="subtitle1">
                <Trans>
                  Spending Limit (coffee per interval):{' '}
                  {mojo_to_coffee_string(limit)}
                </Trans>
              </Typography>
            </Box>
          </Box>
...
const SendCard = (props) => {
  ...
  function send() {
    ...
    const fee = coffee_to_mojo(fee_input.value);
