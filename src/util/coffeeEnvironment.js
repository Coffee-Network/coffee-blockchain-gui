...
const getCoffeeVersion = () => {
  let version = null;
  const exePath = getExecutablePath('coffee');
  // first see if we can get a coffee exe in a standard location relative to where we are
  try {
    ...
    // that didn't work, let's try as if we're in the venv or coffee is on the path
    try {
      version = child_process
